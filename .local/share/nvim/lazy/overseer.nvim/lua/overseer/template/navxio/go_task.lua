...
local provider = {
  ...
  condition = {
    callback = function(opts)
      if vim.fn.executable("go-task") == 0 then
        return false, 'Command "go-task" not found'
      end
  ...
  generator = function(opts, cb)
    local ret = {}
    local cmd = { "go-task", "--list-all", "--json" }
    local jid = vim.fn.jobstart(cmd, {
      ...
      on_stdout = vim.schedule_wrap(function(_, output)
        ...
        for _, target in ipairs(data.tasks) do
          ...
          table.insert(ret, overseer.wrap_template(template, override, { target = target.name, cwd = opts.dir }))
        end
        cb(ret)
      end),
    })
